...
function NewsItemRow({ item, index }: { item: LocalizedNewsItem; index: number }) {
  ...
  return (
    <Reveal delay={Math.min(index * 80, 480)}>
      <article className="border-b border-line py-9 first:pt-0 last:border-b-0">
        ...
        <h2 className="display-heading mt-3 text-[22px] font-semibold leading-[1.3] tracking-tight md:text-[26px]">{item.title}</h2>
        <p className="mt-3 max-w-2xl text-[14.5px] leading-8 text-ink2">{item.summary}</p>
