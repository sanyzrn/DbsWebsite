...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-line bg-surface">
      {/* Colophon wordmark — the monograph closing plate */}
      <div className="wrap overflow-hidden pt-12 md:pt-16" dir="ltr" aria-hidden="true">
        <p className="hero-brand-name select-none whitespace-nowrap text-[clamp(3.4rem,11.5vw,10.5rem)] leading-[0.95] tracking-[-0.02em] text-ink/85">
          Dbs<span className="text-accent">Studio</span>
          <span className="ms-2 inline-block h-[0.09em] w-[0.09em] rounded-[1px] bg-accent align-baseline" />
        </p>
      </div>
          ...
          <div className="flex shrink-0 gap-2">
            {socials.map((s) => (
              <a
                ...
                aria-label={s.label}
                className="hit-min relative flex h-9 w-9 items-center justify-center rounded-full border border-line text-ink2 transition-colors hover:border-hi hover:text-hi"
              >
      ...
      <div className="wrap hidden gap-12 pb-14 pt-4 md:grid md:grid-cols-12">
        <div className="md:col-span-6">
          <p className="mt-2 text-[13.5px] font-bold text-hi">{t.footer.tagline}</p>
          <p className="mt-4 max-w-sm text-[13.5px] leading-7 text-ink2">{t.footer.desc}</p>
          <p className="mt-7 inline-flex items-center gap-2.5 rounded-full border border-line px-3.5 py-2 text-[10px] uppercase tracking-[0.18em] text-ink3" dir="ltr">
        ...
        <div className="md:col-span-3">
          <h3 className="mb-5 font-mono text-[10.5px] font-semibold uppercase tracking-[0.2em] text-ink3">{t.footer.navTitle}</h3>
          <ul className="space-y-3">
            {links.map((l) => (
              <li key={l.to}>
                <Link to={l.to} className="link-underline text-[14px] font-semibold text-ink2 transition-colors hover:text-ink">
                  {l.label}
                </Link>
                {l.to === articlesTo && showNews ? (
                  <Link
        ...
        <div className="md:col-span-3">
          <h3 className="mb-5 font-mono text-[10.5px] font-semibold uppercase tracking-[0.2em] text-ink3">{t.footer.socialTitle}</h3>
          <div className="flex gap-2.5">
            {socials.map((s) => (
              <a
                ...
                rel="noopener noreferrer"
                aria-label={s.label}
                className="flex h-11 w-11 items-center justify-center rounded-full border border-line text-ink2 transition-all duration-300 hover:-translate-y-1 hover:border-hi hover:text-hi"
              >
