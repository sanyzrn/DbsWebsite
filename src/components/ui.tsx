...
export function SectionHead({
  ...
  title: string;
  lead?: string;
  /** Catalogue index, e.g. "01" — rendered as a quiet mono marker. */
  index?: string;
  className?: string;
}) {
  return (
    <div className={cn("max-w-2xl", className)}>
      <Reveal>
        <span className="kicker">
          {index ? (
            <span className="font-mono text-accent" dir="ltr" aria-hidden="true">
              {index}
            </span>
          ) : null}
          {kicker}
      ...
      <Reveal delay={80}>
        <h2 className="display-heading mt-4 text-[28px] font-semibold leading-[1.22] tracking-tight sm:mt-5 sm:text-[34px] md:text-[46px] md:leading-[1.14]">
          {title}
        </h2>
      </Reveal>
      {lead && (
        <Reveal delay={160}>
          <p className="mt-4 text-[14.5px] leading-8 text-ink2 sm:mt-5 sm:text-[15.5px] sm:leading-[1.9] md:text-base">{lead}</p>
        </Reveal>
      )}
    </div>
  );
