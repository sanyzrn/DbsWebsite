...
function Meta({ label, items, mono }: { label: string; items: string[]; mono?: boolean }) {
  return (
    <div>
      <span className="mb-2.5 block font-mono text-[9.5px] font-semibold uppercase tracking-[0.2em] text-ink3">{label}</span>
...
function StatusBadge({ status }: { status?: "production" | "concept" }) {
  ...
  if (status === "concept") {
    return (
      <span className="shrink-0 rounded-full border border-dashed border-ink3/50 px-2.5 py-0.5 font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-ink3">
  ...
  return (
    <article
      className={cn(
        ...
        "group flex min-h-0 w-full flex-1 flex-col overflow-hidden rounded-md border bg-surface transition-colors duration-500 hover:border-hi/50",
      ...
      >
        ...
        {(project.featured || project.status === "concept") && (
          <span className="absolute start-3 top-3 flex flex-wrap gap-1.5" aria-hidden="true">
            {project.featured && project.status !== "concept" ? (
              <span className="rounded-full border border-hi/40 bg-shot/90 px-2.5 py-0.5 font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-accent backdrop-blur">
                {featuredLabel}
              </span>
            ) : null}
      ...
      >
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h3
              className={cn(
                "line-clamp-2 text-[21px] font-extrabold tracking-tight",
                featured && "lg:line-clamp-none lg:text-[34px] lg:font-black"
              )}
          ...
          <p className="mt-2.5 line-clamp-3 text-[13.5px] leading-7 text-ink2">{project.desc}</p>
        </div>
        {showMeta && (
  ...
  return (
    <section id="projects" className="relative overflow-hidden section-pad border-t border-line">
      ...
      <div className={cn("wrap", mode === "full" && "relative")}>
        ...
        {!isTeaser && (
          <div className="mt-10 sm:mt-14" data-testid="projects-grid">
            {featured && (
              <Reveal>
                <article
                  className={cn(
                    "group grid overflow-hidden rounded-md border bg-surface transition-colors duration-500 hover:border-hi/50 lg:grid-cols-5",
                ...
                >
                  ...
                  >
                    <ProjectShot project={featured} className="h-full min-h-[240px] transition-transform duration-[900ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.015] lg:aspect-auto" />
                    {featured.featured && featured.status !== "concept" && (
                      <span
                        className="absolute start-4 top-4 rounded-full bg-shot/90 px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-accent backdrop-blur"
                        aria-hidden="true"
                      >
                        {t.projects.featured}
                      </span>
                  ...
                  <div className="flex flex-col gap-6 p-7 md:p-10 lg:col-span-2">
                    <div>
                      <div className="flex flex-wrap items-center gap-3">
                        <h3 className="text-[30px] font-black tracking-tight md:text-[36px]" dir={featured.id === "hesabyar" ? undefined : "ltr"}>
                          {featured.name}
                        </h3>
                        <StatusBadge status={featured.status} />
                      </div>
                      <p className="mt-2.5 text-[15px] font-bold leading-8 text-hi">{featured.subtitle}</p>
                      <p className="mt-4 text-[14.5px] leading-[1.95] text-ink2">{featured.desc}</p>
                    </div>
            ...
            <div className="mt-8 grid gap-8 sm:mt-10 sm:grid-cols-2 xl:grid-cols-3">
              {grid.map((p, i) => (
                <Reveal key={p.id} delay={(i % 3) * 90} className="h-full">
                  <article
                    className={cn(
                      "group flex h-full flex-col overflow-hidden rounded-md border bg-surface transition-all duration-500 hover:-translate-y-1 hover:border-hi/50",
                      p.status === "concept" ? "border-dashed border-line2" : "border-line"
                    )}
                  >
                    <Link to={detailTo(p.slug)} className="relative overflow-hidden text-start" aria-label={previewLabel(p.name)}>
                      <ProjectShot project={p} className="transition-transform duration-[900ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.035]" />
                      <span
                        className="absolute bottom-3 end-4 font-mono text-[10px] font-semibold tracking-[0.2em] text-white/45"
                        aria-hidden="true"
                        dir="ltr"
                      >
                    ...
                    <div className="flex flex-1 flex-col gap-5 p-6">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="text-[22px] font-extrabold tracking-tight" dir={p.id === "hesabyar" ? undefined : "ltr"}>
                            {p.name}
                          </h3>
                          <StatusBadge status={p.status} />
                        </div>
                        <p className="mt-1.5 text-[13.5px] font-bold leading-7 text-hi">{p.subtitle}</p>
                        <p className="mt-3 text-[13.5px] leading-[1.9] text-ink2">{p.desc}</p>
                      </div>

                      {p.caps && p.caps.length > 0 && (
                        <div>
                          <span className="mb-2.5 block font-mono text-[9.5px] font-semibold uppercase tracking-[0.2em] text-ink3">{t.projects.capsLabel}</span>
        ...
        {isTeaser && (
          <div className="mt-10 flex justify-center sm:mt-12 md:hidden">
            <Link to={localePath(lang, "/projects")} className="btn btn-ghost h-12 px-7 text-[14px]">
              {t.projects.seeAll}
    ...
    <section className="relative overflow-hidden section-pad border-t border-line">
      ...
      <div className="mt-8 max-w-3xl">
        <div className="flex flex-wrap items-center gap-4">
          <h1 className="text-[36px] font-black tracking-tight md:text-[52px] md:leading-[1.05]" dir={project.id === "hesabyar" ? undefined : "ltr"}>
            {project.name}
          </h1>
          <StatusBadge status={project.status} />
        </div>
        <ProjectMetaRow project={project} />
        <p className="mt-4 font-mono text-[11px] uppercase tracking-[0.18em] text-ink3">{project.tags.join(" · ")}</p>
      </div>
      <div className="mt-10 overflow-hidden rounded-md border border-line sm:mt-12">
        <ProjectShot project={project} />
      </div>
      <div className="mt-12 grid gap-10 lg:grid-cols-12 lg:gap-14">
        <div className="lg:col-span-7">
          <p className="text-[19px] font-bold leading-9 tracking-tight text-hi md:text-[22px] md:leading-[1.7]">{project.subtitle}</p>
          <p className="mt-6 max-w-3xl text-[15px] leading-[1.95] text-ink2">{project.desc}</p>
        </div>
        <div className="grid gap-6 sm:grid-cols-2 lg:col-span-5 lg:content-start lg:grid-cols-1">
          <Meta label={t.projects.roleLabel} items={project.role} />
          <Meta label={t.projects.techLabel} items={project.tech} mono />
        </div>
      </div>
      {(project.problem || project.approach || project.result) && (
        <div className="mt-14 grid gap-px overflow-hidden rounded-md border border-line bg-line md:grid-cols-3">
          {project.problem ? (
            <div className="bg-page p-6 md:p-8">
              <span className="mb-3 block font-mono text-[9.5px] font-semibold uppercase tracking-[0.2em] text-ink3">{t.projects.problemLabel}</span>
              <p className="text-[14px] leading-8 text-ink2">{project.problem}</p>
            </div>
          ) : null}
          {project.approach ? (
            <div className="bg-page p-6 md:p-8">
              <span className="mb-3 block font-mono text-[9.5px] font-semibold uppercase tracking-[0.2em] text-ink3">{t.projects.approachLabel}</span>
              <p className="text-[14px] leading-8 text-ink2">{project.approach}</p>
            </div>
          ) : null}
          {project.result ? (
            <div className="bg-page p-6 md:p-8">
              <span className="mb-3 block font-mono text-[9.5px] font-semibold uppercase tracking-[0.2em] text-ink3">{t.projects.resultLabel}</span>
              <p className="text-[14px] leading-8 text-ink2">{project.result}</p>
            </div>
          ) : null}
        </div>
      )}
      {project.caps && project.caps.length > 0 && (
        <div className="mt-12">
          <span className="mb-3 block font-mono text-[9.5px] font-semibold uppercase tracking-[0.2em] text-ink3">{t.projects.capsLabel}</span>
          <ul className="grid gap-x-8 gap-y-3 border-t border-line pt-6 sm:grid-cols-2">
            {project.caps.map((c) => (
              <li key={c} className="flex items-center gap-2.5 text-[13.5px] font-medium text-ink2">
