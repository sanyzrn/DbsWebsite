...
export default function Nav() {
  ...
  const fieldNotesDesktop = (
    <div className="flex flex-col items-start px-3.5 py-1">
      <Link
        to={articlesTo}
        className="link-underline text-[13.5px] font-semibold text-ink2 transition-colors duration-300 hover:text-ink"
      >
        {t.nav.articles}
      </Link>
      {showNews && (
  ...
  const navLinkClass =
    "link-underline rounded-sm px-3.5 py-2 text-[13.5px] font-semibold text-ink2 transition-colors duration-300 hover:text-ink";
    ...
    >
      <div className="wrap flex h-[76px] items-center justify-between gap-4">
        <Link to={home} className="group flex items-center gap-2.5" aria-label={t.nav.homeLinkLabel}>
          ...
          <span
            dir="ltr"
            className="hidden font-[family-name:var(--font-display-en)] text-[17px] font-semibold tracking-tight text-ink sm:inline-block"
          >
            Dbs<span className="text-accent">Studio</span>
          </span>
        </Link>

        <nav className="hidden items-center gap-1 lg:flex" aria-label={t.nav.primaryNavLabel}>
          <Link to={links[0].to} className={navLinkClass}>
            {links[0].label}
          </Link>
          {fieldNotesDesktop}
          {links.slice(1).map((l) => (
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          <button
            ...
            className="flex h-10 items-center gap-1.5 rounded-full border border-transparent px-3 text-[12px] font-bold text-ink2 transition-colors duration-300 hover:border-line hover:text-ink"
          >
          ...
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-full text-ink2 transition-colors duration-300 hover:bg-surface hover:text-ink"
          >
            {theme === "light" ? <Moon className="h-[17px] w-[17px]" strokeWidth={2} /> : <Sun className="h-[18px] w-[18px]" strokeWidth={2} />}
          </button>
          <Link to={ctaTo} className="btn btn-primary ml-1.5 hidden h-10 px-5 text-[13px] md:inline-flex">
            {t.nav.cta}
          </Link>
          <button
            ref={toggleRef}
            ...
            className="flex h-10 w-10 items-center justify-center rounded-full text-ink transition-colors hover:bg-surface lg:hidden"
          >
      ...
      >
        <nav
          id={PANEL_ID}
          className="mx-auto max-w-lg overflow-hidden rounded-md border border-line bg-page shadow-[0_18px_50px_-24px_rgba(0,0,0,0.3)]"
            ...
            {mobilePrimaryLinks.map((l, i) => (
              <span key={l.to}>
                <Link
                  ...
                  to={l.to}
                  onClick={closeMenu}
                  className="group flex items-center justify-between rounded-sm px-3 py-3.5 text-[15px] font-bold tracking-tight text-ink transition-colors hover:bg-surface"
                >
                  <span className="flex items-baseline gap-3">
                    <span className="font-mono text-[10px] font-medium tracking-[0.14em] text-ink3 transition-colors group-hover:text-accent" dir="ltr">
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    {l.label}
                  </span>
                  <span aria-hidden="true" className="text-line2 transition-colors group-hover:text-accent">
                    →
                  </span>
                </Link>
                {l.to === articlesTo && showNews && (
                  <Link
                    to={newsTo}
                    onClick={closeMenu}
                    className="ms-6 flex items-center justify-between rounded-sm px-3 py-2 text-[13px] font-semibold tracking-tight text-ink3 transition-colors hover:bg-surface hover:text-ink"
                  >
                    {t.nav.news}
                  </Link>
                )}
      ...
      {open && (
        <button
          ...
          className="fixed inset-0 top-[76px] z-40 bg-ink/25 backdrop-blur-[1px] lg:hidden"
          onClick={closeMenu}
        />
      )}
    </header>
